...
class OffensiveAStarAgent(CaptureAgent):
    ...
    def get_home_target(self, gameState):
        ...
        is_red = self.red
        border_x = width // 2 - 1 if is_red else width // 2  # Left border for red, right for blue

        # Find the closest non-wall position to the midpoint
        mid_y = height // 2
        for offset in range(height // 2 + 1):  # Check progressively farther positions
            # Check upwards from mid_y
            up_y = mid_y + offset
            if up_y < height and not walls[border_x][up_y]:
                return (border_x, up_y)
            
            # Check downwards from mid_y
            down_y = mid_y - offset
            if down_y >= 0 and not walls[border_x][down_y]:
                return (border_x, down_y)

        # If no non-wall positions are found (extremely unlikely), default to the midpoint
    ...
    def heuristic(self, pos, food, gameState):
        """
        Heuristic to evaluate positions for food collection, capsule targeting, and ghost avoidance.
        - Encourages food collection.
        - Prioritizes capsules when available.
        - Avoids ghosts by penalizing proximity.
        """
        ...
        # Filter for actual ghosts (is_pacman = False)
        active_ghosts = [
            g.get_position() for g in ghosts if not g.is_pacman and g.get_position() is not None
        ]
        print(f"nuber of ghost: {len(active_ghosts)}")
            # Penalize proximity to ghosts if they are on the other quadrant
        if self.on_own_side(pos, gameState):
            # assign good heuristic
            ghost_penalty = 0

        else:
            ghost_penalty = sum(1 / (1 + self.get_maze_distance(pos, ghost)) for ghost in active_ghosts if ghost)

        # Capsule prioritization (encourage when available)
        capsules = self.get_capsules(gameState)
        capsule_bonus = 0
        if capsules:
            # Prefer closer capsules
            closest_capsule_dist = min(self.get_maze_distance(pos, capsule) for capsule in capsules)
            capsule_bonus = 200 / (1 + closest_capsule_dist)  # Inverse scaling

        # Combine heuristic components with tuned weights
        return 1*food_dist - 2*capsule_bonus + 3*ghost_penalty
    ...
    def divide_food(self, food, gameState):
        ...
        if check == True:  # Agent 0, 2, etc.
            my_food = [f for f in food if f[1] < mid]  # Bottom half
        else:  # Agent 1, 3, etc.
